...
Phase A: Add parent composite tenant unique constraints on (tenant_id, id) for 9 parent tables:
...
def upgrade() -> None:
    # Add unique constraints on (tenant_id, id) for Phase A parent models
...
def downgrade() -> None:
    # Drop unique constraints created by Phase A
